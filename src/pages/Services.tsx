    ...
    const fetchContent = async () => {
      try {
        ...
        if (docSnap.exists()) {
          ...
          if (servicesBlock) {
            const { title, description, values } = servicesBlock.content;
          ...
          if (processBlock) {
            const { title, description, values } = processBlock.content;
            setProcessTitle(title || '');
            setProcessDescription(description || '');
            setProcessSteps(values || []);
          }

          if (ctaBlock) {
            setCta(ctaBlock.content);
          }
        }
      } catch (error) {
    ...
    <Layout>
      ...
      <section className="py-16 bg-gray-900 text-white" style={design.darkColor}>
        <div className="container mx-auto px-4 text-center">
          ...
          <p className="body-large text-gray-300 max-w-2xl mx-auto mb-12">
            {processDescription}
          </p>
        ...
        <div className="container mx-auto px-4 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <Button asChild variant="outline" size="lg">
              <Link to="/projects">צפה בפרויקטים</Link>
            </Button>
          </div>
        </div>
      </section>
